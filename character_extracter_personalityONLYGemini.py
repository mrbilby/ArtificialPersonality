...
def api_call(book_detail):
    ...
    prompt = f"""You are a novel critic and know everything about authors and their impact. Please detail 
                 the cultural impact, how this author writes  different from others, detail about the author and 
                 similar books to read. The author is {book_detail}"""

    response = model.generate_content(prompt)  # Generate content based on the prompt
    return response.text.strip()  # Return the cleaned-up response text

class CharacterPersonalityExtractor:
    def __init__(self, openai_api_key: str, token_limit: int = 600000):
    ...
    def generate_personality(self, character_analysis: str) -> Dict:
        ...
        try:
            ...
            prompt = f""""You are a character analyst creating personality profiles. Return only the JSON object with no formatting tokens, no ```json markers, and no backticks. {final_prompt}"""
    ...
    def _get_character_analysis(self, epub_content: str, character_name: str) -> str:
        ...
        prompt = f""""You are a literary analyst specialized in character analysis {final_prompt}"""
...
def main():
    ...
    token_limit = 600000  # Set default token limit
